...
function ensureMarketplace(name: string, source: MarketplaceSource): string | null {
  ...
  if (!url) {
    ...
    log.warn('Unsupported marketplace source', { name, source });
    return null;
  }
  ...
  if (fs.existsSync(path.join(dir, '.git'))) {
    try {
      ...
      log.debug('Marketplace updated', { name });
    } catch {
      log.warn('Marketplace pull failed — using cached version', { name });
    }
  } else {
    try {
      ...
      log.info('Marketplace cloned', { name, url });
    } catch (err) {
      log.error('Failed to clone marketplace', { name, url, err });
      return null;
    }
  }
...
export function syncPluginsForRepo(repo: string): string[] {
  ...
  for (const [pluginRef, enabled] of Object.entries(settings.enabledPlugins)) {
    ...
    if (atIdx === -1) {
      log.warn('Invalid plugin reference — missing @marketplace', { pluginRef });
      continue;
    }
    ...
    if (!marketplaceDir) {
      log.warn('Marketplace not available', { pluginRef, marketplaceName });
      continue;
    }

    const pluginDir = findPluginInMarketplace(marketplaceDir, pluginName);
    if (!pluginDir) {
      log.warn('Plugin not found in marketplace', { pluginRef, marketplaceDir });
      continue;
    }
    ...
    log.info('Plugin resolved', { pluginRef, pluginDir });
  }

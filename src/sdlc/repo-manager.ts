...
export function ensureRepoCloned(repo: string): string {
  ...
  log.info('Cloning repo for SDLC', { repo, dir });
 ...
 */
export function createWorktree(repo: string, issueNumber: number, branch?: string): string {
  ...
  if (fs.existsSync(wtPath)) {
    if (!branch) {
      try {
        ...
        log.debug('Updated existing worktree to latest origin/main', { repo, issueNumber });
      } catch (err) {
        log.warn('Failed to update worktree', { repo, issueNumber, err });
      }
    }
    return wtPath;
  }
  ...
  log.info('Created worktree', { repo, issueNumber, wtPath, branch });
  return wtPath;
}
...
function installGitGuardrails(wtPath: string, repo: string): void {
  ...
  try {
    ...
    log.debug('Git identity set from repo history', { repo, name, email });
  } catch {
  ...
  try {
    ...
    if (gitContent.startsWith('gitdir:')) {
      ...
      const absGitdir = path.isAbsolute(gitdir) ? gitdir : path.resolve(wtPath, gitdir);
 ...
 */
export function switchWorktreeToBranch(repo: string, issueNumber: number, branch: string): void {
  const mainDir = ensureRepoCloned(repo);
  const wtPath = getWorktreePath(repo, issueNumber);
  ...
  log.info('Switched worktree to branch', { repo, issueNumber, branch });
}
...
export function rebaseWorktree(repo: string, issueNumber: number): boolean {
  ...
  try {
    ...
    log.info('Rebased worktree onto origin/main', { repo, issueNumber });
    return true;
  } catch {
    ...
    log.warn('Rebase failed — conflicts detected', { repo, issueNumber });
    return false;
  }
}
...
export function removeWorktree(repo: string, issueNumber: number): void {
  ...
  try {
    ...
    log.info('Removed worktree', { repo, issueNumber });
  } catch (err) {
    log.warn('Failed to remove worktree', { repo, issueNumber, err });

...
export function addFlag(repo: string, number: number, flag: SdlcFlag): void {
  ...
  try {
    execSync(
      `gh api repos/${repo}/issues/${number}/labels -X POST -f "labels[]=${label}"`,
      { env: ghEnv(), stdio: 'pipe' },
    );
    logger.info({ repo, number, flag }, 'Flag added');
  } catch (err) {
    logger.warn({ repo, number, flag, err }, 'Failed to add flag');
  }
}
...
export function removeFlag(repo: string, number: number, flag: SdlcFlag): void {
  ...
  try {
    execSync(
      `gh api repos/${repo}/issues/${number}/labels/${encodeURIComponent(label)} -X DELETE`,
      { env: ghEnv(), stdio: 'pipe' },
    );
    logger.info({ repo, number, flag }, 'Flag removed');
  } catch {
